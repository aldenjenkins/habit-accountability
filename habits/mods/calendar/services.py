...
def generate_stats_dict(completion_entries):
    ...
    for entry in completion_entries:
        if entry.did_complete and entry.create_ts > entry.habit.create_ts:
            stats[timezone.localtime(entry.create_ts).month][
                timezone.localtime(entry.create_ts).day
            ]["completed_habits"].append(entry.habit)
...
class CalendarMixin:
    ...
    def __init__(self, save_path: str):
        ...
        self.rows = (self.top_padding + 6) * (
            self.num_calendars_to_show / self.num_calendars_per_row
        )
    ...
    def add_horizontal_lines(self, col, row):
        self.draw.line(
            xy=[
                (self.col_size * col, (self.row_size * row - 4) - self.line_size * 2),
                (
    ...
    def draw_week_day_names(self, col, row):
        for c, week in enumerate(WEEK):
            ...
            y = self.row_size * row - (self.title_size + 4)
            self.draw.text(
                (x, y),
                week,
                font=ImageFont.truetype(
...
class AccountabilityMonthlyCalendar(CalendarMixin):
    ...
    def draw_day_border(self, col, row):
        ...
        self.draw.line(
            xy=[
                ...
                (
                    self.col_size * col + self.col_size,
                    row * self.row_size + self.row_size,
                ),
            ],
            fill=(0, 0, 0),
    ...
    def draw_habits(self, col, row, month, day):
        ...
        for habit in self.accountability_stats[month][day]["completed_habits"]:
            ...
            if (print_row + self.habit_size + self.day_size) >= (
                row * self.row_size + self.row_size
            ):
                rel_row = 0
                rel_col += 1
    ...
    def add_month_name(self, name, col, row):
        self.draw.text(
            (
                self.col_size * col - 1,
                self.row_size * row - self.top_padding * self.row_size,
            ),
            name,
            fill=(
                0,
